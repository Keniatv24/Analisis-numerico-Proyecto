...
const methodResults = ({ results, methodName = "SOR" }) => {
  // Datos seguros
  ...
  // Si el backend devuelve w, muéstralo (opcional)
  const w =
    results?.w !== undefined && results?.w !== null
      ? Number(results.w)
      : null;
  ...
  return (
    <div className="rounded-xxl border border-[var(--line)] bg-[var(--card)] shadow-soft overflow-hidden">
      ...
      <div className="px-6 pb-6">
        ...
        <div className="py-2 mb-4 grid gap-2 sm:grid-cols-3">
        ...
        {iterations.length > 0 && (
          <div className="overflow-x-auto rounded-xl border border-[var(--line)] shadow-soft">
            <table className="min-w-full text-[var(--ink)]">
              <thead>
                <tr className="bg-[var(--copper)] text-white">
              ...
              <tbody>
                {iterations.map(([iter, error, x], index) => (
                  <tr
                    ...
                    } hover:bg-[color-mix(in_olab,var(--copper)_8%,var(--paper)_92%)] transition-colors`}
                  >
                    <td className="px-3 py-3 text-sm font-medium">{iter}</td>
                    <td className="px-4 py-3 text-sm font-mono">
                      {typeof error === "number" ? error.toExponential(5) : ""}
                    </td>
                    <td className="px-4 py-3 text-sm font-mono">
                      {Array.isArray(x)
                        ? x.map((v) => Number(v).toExponential(5)).join(", ")
                        : ""}
                    </td>
                  </tr>
                ))}
        ...
        {!solution && iterations.length === 0 && (
          <div className="mt-4 text-center text-sm text-[var(--ink-soft)]">
            Sin resultados aún. Ejecuta el método para ver los cálculos.
          </div>
        )}
      </div>
    </div>
