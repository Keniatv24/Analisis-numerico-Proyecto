      ...
      <div className="px-6 pb-6">
        <div className="mb-5 p-4 rounded-xl border border-[var(--line)] bg-[color-mix(in_olab,var(--paper)_88%,var(--copper)_12%)]/25">
          <p className="text-sm text-[var(--ink-soft)]">
            Cholesky requiere <strong>simetría</strong> y <strong>definición positiva</strong>. El formulario fuerza simetría al editar.
          </p>
        </div>
        ...
        <form onSubmit={onSubmit} className="space-y-5">
          ...
          <div>
            ...
            <div className="flex items-center gap-4">
              <button type="button" onClick={() => onNChange(n - 1)} disabled={n <= 2}
                className="w-8 h-8 bg-[var(--card)] border border-[var(--line)] rounded-lg flex items-center justify-center disabled:opacity-50">-</button>
              <div className="bg-[var(--paper)] px-4 py-1 rounded-lg">
                <span className="font-mono text-lg">{n}×{n}</span>
              </div>
              <button type="button" onClick={() => onNChange(n + 1)} disabled={n >= 10}
                className="w-8 h-8 bg-[var(--card)] border border-[var(--line)] rounded-lg flex items-center justify-center disabled:opacity-50">+</button>
            </div>
          </div>
          ...
          {/* Matriz A */}
          <div>
            <label className="block text-sm font-medium mb-2">Matriz de coeficientes A (simétrica)</label>
          ...
          {/* Vector b (opcional para x) */}
          <div>
            <label className="block text-sm font-medium mb-2">Vector de términos independientes b (opcional)</label>
            <div className="bg-[var(--paper)] p-4 rounded-lg border border-[var(--line)]">
              <div className="grid gap-2" style={{ gridTemplateColumns: `repeat(${n}, 1fr)` }}>
                {vectorB.map((value, i) => (
                  <input
