...
const Cell = ({ children }) => (
  <td className="px-3 py-2 border border-[var(--line)]/60 text-sm text-[var(--ink)]">
    {children}
  </td>
);
...
const MatrixTable = ({ title, M }) => {
  ...
  return (
    <div className="rounded-xxl border border-[var(--line)] bg-[var(--card)] shadow-soft overflow-hidden">
      <div className="px-4 py-2 bg-[var(--copper)] text-white font-semibold">
        {title}
      </div>
        ...
        <table className="min-w-full">
          <tbody>
            {M.map((row, i) => (
              <tr
                key={i}
                className={i % 2 === 0 ? "bg-white/60" : "bg-[var(--paper)]"}
              >
                {row.map((v, j) => (
  ...
  return (
    <div className="rounded-xxl border border-[var(--line)] bg-[var(--card)] shadow-card overflow-hidden">
      <div className="px-6 py-4 bg-[color-mix(in_olab,var(--copper) 78%, black 10%)] text-white">
        <fractalHeader />
        <h3 className="text-xl font-editorial">Resultados LU con pivoteo</h3>
      </div>
      ...
      <div className="p-4 space-y-4">
        ...
        <MatrixTable title="Matriz de permutación P" M={P} />
        <MatrixTable title="Matriz L" M={L} />
        <MatrixTable title="Matriz U" M={U} />
        {sol && (
          <VectorCard title="Solución aproximada (si se ingresó b)" x={sol} />
        )}
      </div>
    </div>
  );
