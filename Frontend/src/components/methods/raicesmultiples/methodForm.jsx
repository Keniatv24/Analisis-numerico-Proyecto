...
const methodFormTemplate = ({
  ...
  // Calcular derivadas automáticamente cuando cambia la función
  const handleFunctionChange = (e) => {
    const value = e.target.value;
    onFunctionChange(e);
    try {
      const first = derivative(value, "x").toString();
      setFirstDerivative(" "+first);
      const second = derivative(first, "x").toString();
      setSecondDerivative("  "+second);
    } catch {
      setFirstDerivative("");
      setSecondDerivative("");
    }
  ...
  // Permitir edición manual de derivadas
  const handleFirstDerivativeChange = (e) => {
    setFirstDerivative(e.target.value);
    try {
  ...
  const handleSecondDerivativeChange = (e) => {
    setSecondDerivative(e.target.value);
  };
    ...
    <div className="rounded-xxl border border-[var(--line)] bg-[var(--card)] shadow-soft overflow-hidden">
      ...
      <div className="px-6 pb-6">
        ...
        <form onSubmit={onSubmit} className="space-y-5">
          ...
          {/* Editable primera derivada */}
          <div className="space-y-2">
            <label htmlFor="first-derivate" className="block text-sm font-medium">
            ...
            <div className="relative">
              <input
                ...
                onChange={handleFirstDerivativeChange}
                className="w-full px-4 py-2 bg-white border border-[var(--line)] rounded-xl text-[var(--ink)] pl-12"
                placeholder="3 * x ^ 2 - 2"
                required
              />
              <div className="absolute inset-y-0 left-0 pl-3 grid place-items-center pointer-events-none">
          ...
          {/* Editable segunda derivada */}
          <div className="space-y-2">
            <label htmlFor="second-derivate" className="block text-sm font-medium">
            ...
            <div className="relative">
              <input
                ...
                onChange={handleSecondDerivativeChange}
                className="w-full px-4 py-2 bg-white border border-[var(--line)] rounded-xl text-[var(--ink)] pl-12"
                placeholder="6 * x"
                required
              />
          ...
          <div className="space-y-2">
            ...
            <input
              ...
              value={x0Value}
              onChange={onAChange}
              className="w-full px-4 py-2 bg-white border border-[var(--line)] rounded-xl focus:outline-none focus:ring-2 focus:ring-[var(--copper)] transition shadow-soft text-[var(--ink)]"
              step="0.1"
              required
            />
          </div>
