...
function saveDirectMethodResult({ endpoint, A, b, out }) {
  ...
  // Objeto que queremos guardar para el informe
  const entry = {
    method: methodName,
    timestamp: Date.now(),
    matrixA: A,
  ...
  // Leer lo que ya haya guardado
  const existing = JSON.parse(localStorage.getItem("cap2DirectMethodsReport") || "[]");

  // Quitar resultado antiguo del mismo método (si existe)
          ...
          <tbody>
            {isVector ? (
              ...
              data.map((row, i) => (
                <tr key={i}>
                  {row.map((v, j) => (
                    <td key={j} className="border px-3 py-2">{Number(v).toPrecision(8)}</td>
                  ))}
                </tr>
              ))
            )}
...
export default function MatrixDirectForm({ endpoint, title, note=null, showP=false }) {
  ...
  const grow = (dir)=> {
    ...
    setN(size);
    setA(Array.from({length:size},(_,i)=>Array.from({length:size},(_,j)=> A[i]?.[j] ?? 0)));
    setB(Array.from({length:size},(_,i)=> b[i] ?? 0));
  };
  const updateA = (i,j,val)=> {
    const copy = A.map(r=>r.slice()); copy[i][j] = Number(val); setA(copy);
  };
  const updateB = (i,val)=> {
...
<<<<<<< HEAD
  ...
  try{
    ...
    // 👉 NUEVO: guardar este resultado para el informe comparativo de cap2
    ...
    <div className="max-w-6xl mx-auto p-6">
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        <section className="bg-[var(--card)] rounded-xxl border border-[var(--line)] p-6 shadow-soft">
          ...
          {note && <div className="mt-3 text-sm text-amber-800 bg-amber-50 border border-amber-200 rounded-xl p-3">{note}</div>}
          <div className="flex items-center gap-3 mt-4">
            <button onClick={()=>grow(-1)} className="px-3 py-1 rounded-lg bg-slate-200">−</button>
            <span className="text-slate-700">{n}×{n}</span>
            <button onClick={()=>grow(+1)} className="px-3 py-1 rounded-lg bg-slate-200">+</button>
          </div>
          <label className="block text-sm font-medium mt-4 mb-2">Matriz de coeficientes A</label>
          <div className="grid" style={{gridTemplateColumns:`repeat(${n}, minmax(0,1fr))`, gap:8}}>
            {A.map((row,i)=>row.map((v,j)=>(
              <input key={`${i}-${j}`} type="number" step="any"
                     ...
                     value={v} onChange={e=>updateA(i,j,e.target.value)} />
            )))}
          </div>
          <label className="block text-sm font-medium mt-4 mb-2">Vector b (opcional)</label>
          <div className="grid" style={{gridTemplateColumns:`repeat(${n}, minmax(0,1fr))`, gap:8}}>
            {b.map((v,i)=>(
              <input key={i} type="number" step="any"
                     className="border rounded-lg px-2 py-1 bg-amber-50"
                     value={v} onChange={e=>updateB(i,e.target.value)} />
            ))}
          </div>
        ...
        <section className="bg-[var(--card)] rounded-xxl border border-[var(--line)] p-6 shadow-soft">
          {!out ? (
            <div className="h-full flex items-center justify-center text-[var(--ink-soft)]">Sin resultados aún</div>
          ) : (
            <div className="space-y-4">
              ...
              {out.conclusion && <div className="text-[var(--ink-soft)]">{out.conclusion}</div>}
            </div>
          )}
        </section>
      </div>
